...
class DasoucheProxyMiddleware(object):
    def __init__(self):
        self.count = 0
    ...
    def process_request(self, request, spider):
        if spider.name in ['dasouche_car', 'dasouche_gz', 'dasouche_url']:
            # if self.count < 2:
            #     request.meta['proxy'] = self.proxy
            #     self.count += 1
            # else:
...
class DasoucheUserAgentMiddleware(UserAgentMiddleware):
    def process_request(self, request, spider):
...
class DasoucheSpiderMiddleware(object):
...
class DasoucheDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
